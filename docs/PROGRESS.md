...
| 任务 | 24 / 77（P0.1-0.5 + P1.1-1.5 + P2.1-2.5 + P3.1 + P3.2 + P4.1-P4.5 + P4.5b + P4.3b） |
| 文件级（FILETRACK） | DONE 34 / MERG 48 / ACTV 20 / TODO 377（余 SKIP 54，合计 533） |
| 脚本可用率（P7 起跟踪） | - |
...
- [x] GMS-P4.5 聊天（公屏/表情/私聊/找人）✅ 2026-09-12（提交 c75f0a4）
...
- [x] GMS-P4.3b 地图实例数据（Map.wz `info`/`life`/`foothold`/`portal` = Java `MapleMapFactory`）✅ 2026-09-12
  - **`internal/mapp` 新 4 文件**：`mapdata.go`（`Portal`/`Foothold`/`FootholdTree`/`LifeSpawn`/`MapData` + 访问器 + `PortalMap=2`/`PortalDoor=6`/`firstDoorPortal=128`/`NoTargetMap=999999999` + `FieldLimitType` 全 15 位的移植与 `Check`）、`portal.go`（portal 装载与 `Portal`/`PortalByName`/`FindClosestSpawnPoint`）、`foothold.go`（平坦化的 foothold 树 + `FindBelow`/`CalcPointBelow`）、`factory.go`（`MapImagePath`/`LoadData`/`Factory`）
  - **`info/link` 必须解析（27% 的地图是桩）**：Map.wz 4260 张图里 **1152 张是"只有 `info` + `link`"的小桩**（如 105090321 只有 1135 字节，本体在 105090320）；Java 用 `getIntConvert` 读 **string** 形态的 link 再读目标图，**只跳一跳**。Go 保真：`ID` 保持请求的 id、`ImageID` 记录真正用的图；`link==0`/`link==自身`/目标是 link 都只告警不递归（实测真实数据 0 条链式 link）
  - **portal id 规则（易错）**：`pt == 6`（DOOR_PORTAL）**忽略 wz 节点名**，从 **128** 起按文档序编号；其余用 `Integer.parseInt(节点名)`。所以 map 100000000 的 6 个 `tp`（节点名 28..33）实际 id 是 **128..133**，`Portal(28..33)` 全是 nil
  - **foothold 树在 Java 里从不细分**：`lBound/uBound` 从 (0,0) 起按 min/max 扩张 ⇒ 每个 foothold 都满足根节点包含判定 ⇒ 四叉树永远只有根一层。Go 直接用**文档序平坦切片**（保真且更快）。`FindBelow` 保真的四处：① `x1 <= x <= x2 && x1 != x2` 过滤（**墙永不当地板**，且真实数据里 410/359848 条 foothold 是 `x1 > x2` 存的，Java 永远匹配不到——Go 保留这个行为）；② `compareTo` 是**单边**比较器（`y2<o.y1 → -1`、`y1>o.y2 → 1`、否则 0），用 `sort.SliceStable` 复现 TimSort 的 tie 顺序；③ 斜面的 `s5 = cos(atan(s2/s1)) * (s4/cos(atan(s1/s2)))` **连 double 运算与截断 `(int)` 一起照搬**——45° 段在中点算出来是 `49.99999999999999289`，截断成 **49**（比几何值低 1 像素），Java 也这样；④ 先按 y 判"地板在查询点上方就跳过"（y 向下增长）
  - **`life`**：`type` 大小写不敏感（`m`/`n`，其它跳过+告警，对齐 `MapleLifeFactory.getLife`）、`mobTime` **保持秒**（`getInt("mobTime", life, 0)` ⇒ **缺节点 = 0 = 立即重生**，数据里的 -1 才是"只刷一次"；1857 条怪没有该节点）、`hide` 仅对 NPC 生效、`team` 本数据 0 条（Java 默认 -1）、地图 910000000 屏蔽 9310059/9310022 两个向导 NPC、`f` 节点缺失时 `HasF=false`（对齐 Java 的 nil 保护）
  - **怪物出生点 = `calcPointBelow(X,Y).y - 1`（斜面取插值）**，不是 `FindBelow().Y1 - 1`；无 foothold 时 Java **NPE**，Go 保留原始 (x,y) + 告警（否则零 foothold 的地图会整张丢怪）
  - **整份导出实测（一次性工具，已删）**：**4259 张图 0 错误**、1152 张 link 桩全部解析、406,326 条 foothold、**48,456 个怪物出生点 100% 命中 foothold**（Java 的 NPE 分支在真实数据上根本不会触发，raw 兜底计数为 0）、4133 个 NPC
  - **Java bug 不移植**：`MapleMapFactory:146-147`（与 `:329-330`）把 `addMonsterSpawn` **调用两次**（导致 `monsterSpawn` 翻倍、`maxRegularSpawn` 失真、重生时间减半）——Go 每条 life 只装一次；`destroyMap`（返回反转 + 无条件移除）不移植
  - **接线**：`mapp.Map` 加 `data *MapData` + `NewWithData`（`New(id)` 保留，P4.3/P4.5 的 4 个既有测试未动）；`channel.Server` 加 `SetWZ(root *wzs.Root)`（**可空**：nil/无 Map.wz 只告警，地图退化为裸实例，不阻断启动）与 `mapData(id)`，`ChannelServer.Map(id)` 首次创建时装载数据；`cmd/gms` 在 `channel.New` 后 `chs.SetWZ(wzRoot)`
  - **有意偏差（已注记）**：`Factory` 改缓存 **`MapData`**（Java 缓存的是 `MapleMap` 实例；Go 的 `*mapp.Map` 仍按频道各自持有）、并加**失败负缓存**（Java 每次 getMap 都重读）；`PortalByName` 用文档序（Java 是 HashMap 顺序）；`Factory` 每 Server 一个（Java 每频道一个，共享不可变数据是安全的）；不建模 Java `getMap(id, respawns, npcs, reactors)` 的三个开关（`LoadData` 恒读 mobRate 与全部 life，NPC 过滤走 `NPCs()`）
  - **本任务范围外（确认未动）**：reactor、`MapleNodes`/platform/area、`ladderRope`、地图特效、`back`/`tile`/`obj`、区域 BOSS（`addAreaBossSpawn`）、`CreateInstanceMap`、时钟/船、`HealMap`/`DeStorymaps`、DB 驱动的 `customLife`、String.wz 地图名（P3.3 已装）、`info` 的 lvForceMove/decHP/recover 等未消费字段，以及 portal 的 delay/hideTooltip/onlyOnce/horizontalImpact/image 与 foothold 的 forbidFallDown/force（Java 也不读）
  - 测试：`internal/mapp` 新增 16 个（`MapImagePath`/info 标志/裸图降级/link 解析与三种护栏/life 解析与降级/斜面与上斜面插值/墙与 tie 的 `FindBelow`/工厂缓存与负缓存/fieldLimit/map0 夹具/真实树抽样），合计 20 个；`internal/channel` +1（`Map(id)` 同 id 同实例、`Data()` 非 nil、`SetWZ(nil)` 不 panic）；手写夹具 3 个共 3052 字节挂在 `internal/mapp/testdata/wz/Map.wz/Map/Map9/`
  - **live 冒烟全过**（SQLite + 真实 `wz/`）：服务端启动日志 `map data wired wz=Map.wz`；建角 `43333`(id19) 直连 7575 → `WARP_TO_MAP` + `SPAWN_PLAYER` 正常、**全程无 WARN/ERROR**（即真实 map 0 的 info/portal/foothold 解析干净）；测完删角删号停进程
  - **尚未消费的数据**（留给后续）：`Portal`/`ReturnMapID`/`ForcedReturnID` 还没接进登录出生点（`WARP_TO_MAP` 的 spawn 坐标仍是 (0,0)，P4.3 的既有偏差）、`CHANGE_MAP`/`CHANGE_MAP_SPECIAL` handler 未写（换图仍不可用）、`LifeSpawn` 等待 P6 的怪物生成、`FieldLimit` 等待各 handler
- [x] GMS-P4.5b configvalues 开关层（`gui/Start.ConfigValuesMap`，`SELECT * FROM configvalues`）✅ 2026-09-12（提交 c75f0a4，与 P4.5 同一提交：chat.go 的门禁依赖 P4.5b 的 Server 侧实现，拆开会让中间那次不可编译）
...
- 2026-09-12 P4.5 完成（聊天）[提交 c75f0a4]：新增 `internal/packet/chat.go`（`getChatText`/`facialExpression`/`getWhisper`/`getWhisperReply`/`getFindReply`/`getFindReplyWithMap` 逐字段保真）与 `internal/channel/chat.go`（`ChatHandler.GeneralChat` + `PlayerHandler.ChangeEmotion` + `ChatHandler.Whisper_Find` 的 mode 5/68/6），`OnPacket` 接 `GENERAL_CHAT(0x2D)`/`FACE_EXPRESSION(0x2F)`/`WHISPER(0x75)`；`internal/mapp` 加 `BroadcastRanged` + `MaxViewRangeSq`（= MapleMap 的 `broadcastMessage(packet, rangedFrom)` Point 重载，**有**视野过滤且**含**发送者——公屏靠它让说话人看到自己那行，而表情/移动走 boolean 重载：无限视野 + 排除自己）。**顺带修掉一个跨 goroutine 数据竞态**：`BroadcastRanged` 会读别的玩家的坐标，故 `channel.Player` 的坐标态改由 `sync.RWMutex` 保护（`Stance` 字段因此改名 `stance` + 导出访问器）。**范围变更**：原 PLAN 的"关键字屏蔽（abc/关键字屏蔽 表）"经 jar 字节级查证确认为**死代码**（全 jar 无引用、ini 路径写错且文件缺失、唯一编进去的 `Game.屏蔽文字` 硬编码且无人调用），**用户拍板不实现**，并删掉从未接线的 `chat_filter` 配置项。
- 2026-09-12 P4.3b 完成（地图实例数据）[提交待填]：`internal/mapp` 新增 `mapdata.go`/`portal.go`/`foothold.go`/`factory.go` —— Map.wz 的 `info`（returnMap/forcedReturn/fieldLimit/mobRate/…）、`portal`（**pt=6 忽略节点名、从 128 起编号**）、`foothold`（**Java 四叉树从不细分 ⇒ 文档序平坦切片**；`FindBelow` 的墙过滤、单边 `compareTo` + 稳定排序、斜面插值连 double 截断一起照搬）、`life`（大小写不敏感的 m/n、mobTime 秒、910000000 屏蔽两个向导 NPC）；**`info/link` 桩解析必做**（4260 张图里 1152 张是桩）；怪物出生点 = `calcPointBelow().y - 1`；Java 的 `addMonsterSpawn` 双调用 bug 与 `destroyMap` 不移植。`mapp.Map` 加 `NewWithData`/`Data()`，`channel.Server` 加可空 `SetWZ`，`cmd/gms` 接线。整份导出实测 4259 张图 0 错误、48,456 个怪出生点全部命中 foothold；`internal/mapp` 测试 20 个（新增 16）+ `channel` +1；live 冒烟 `map data wired` 且全程无 WARN。
- 2026-09-12 P4.5b 完成（configvalues 开关层）[提交 c75f0a4]：查证纠正——`configvalues` 表**有真实数据**（权威库 322 行），此前"不在 dump"指的是表数据没进 SQLite。新增 `database.ConfigValues` DAO（`SELECT name,val FROM ConfigValues`）+ SQLite 建表（空表 = 全 0 = 全开）+ `channel.Server` 可选装载 + `cmd/gms` 启动期读取；判据沿用 Java 的 **`val > 0` = 关闭**，把 `玩家聊天开关`（关聊天回 `serverNotice(1,"管理员从后台关闭了聊天功能")`）与 `游戏找人开关`（关找人只拦 WHISPER mode 5/68，回 `dropMessage(5,"找人功能被关闭")`）转正。`聊天记录开关` 依赖尚未移植的文件日志，留给运营侧。
